{foreach $product_features as $feature}
    {if $feature.feature_type != "ProductFeatures::GROUP"|enum}
        ...
        <div class="ty-product-feature__value">
            ...
            {if $feature.value === "YesNo::YES"|enum}{__("yes")}{else}{__("no")}{/if}
{*            <span class="ty-compare-checkbox">{if $feature.value === "YesNo::YES"|enum}<i class="ty-compare-checkbox__icon ty-icon-ok"></i>{/if}</span>*}
            ...
            {elseif $feature.feature_type == "ProductFeatures::MULTIPLE_CHECKBOX"|enum && $feature.variants}
                ...
                {foreach from=$feature.variants item="var" name="foo"}
                    ...
                    {if $var.selected}<li class="ty-product-feature__multiple-item">{include file="views/products/components/ab__similar_filter.tpl" feature=$feature variant_id=$var.variant_id}{if !$hide_variant_affix}<span class="ty-product-feature__prefix">{$feature.prefix}</span>{/if}{$var.variant}{if !$hide_variant_affix}{if !$smarty.foreach.foo.last}{if !$feature.filter_id},{/if}{/if}<span class="ty-product-feature__suffix">{$feature.suffix}</span>{/if}</li>{/if}
                {/foreach}
                </ul>
            {elseif in_array($feature.feature_type, ["ProductFeatures::TEXT_SELECTBOX"|enum, "ProductFeatures::EXTENDED"|enum, "ProductFeatures::NUMBER_SELECTBOX"|enum])}
                ...
                {foreach from=$feature.variants item="var"}
                    {if $var.selected}{include file="views/products/components/ab__similar_filter.tpl" feature=$feature variant_id=$var.variant_id}{if $feature.filter_style == "ProductFilterStyles::COLOR"|enum && $var.color}<div class="abt__ut2_color_mark" style="background-color: {$var.color};width:15px;height:15px;display:inline-block;margin-right:5px;margin-top:-2px;border-radius: 50%;{if $var.color == "#ffffff"}border: 1px solid{/if}"></div>{/if}{$var.variant}{/if}
                {/foreach}
