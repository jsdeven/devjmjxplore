		...
		<div{if $promotion.to_date && $promotion.to_date > $smarty.now} class="col1"{/if}>
	        ...
	        <div class="actions-link">
                ...
                <a href="{"promotions.view?promotion_id=`$promotion.promotion_id`"|fn_url}" title="" target="_blank">
                    {if $promotion.short_description|strip_tags|fn_string_not_empty}<i class="cm-tooltip ty-icon-help-circle" title="{$promotion.short_description}"></i>{/if}<span>{__('ab__dotd.detailed')}</span>
                </a>
			</div>
        </div>
