...
$condition .= db_quote(' AND ?:ab__video_gallery.status = ?s', ObjectStatuses::ACTIVE);
...
$parent_product_id = $variations_product_map->getParentProductId($product_id);
...
$master_product_id = $master_producs_repo->findMasterProductId($product_id);
if (!empty($master_product_id)) {
$product_id = $master_product_id;
}
}
}
$condition .= db_quote(' AND ?:ab__video_gallery.product_id = ?i', $product_id);
$product_videos = db_get_hash_array('SELECT ?p FROM ?:ab__video_gallery
...
WHERE ?p ORDER BY ?:ab__video_gallery.pos ASC', 'video_id', implode(', ', $fields), $lang_code, $condition);
...
$markup_items['product']['subjectOf'][] = [
'@type' => 'VideoObject',
'name' => $video['title'],
'description' => $description,
'uploadDate' => date('c', $product_data['timestamp']),
'thumbnailUrl' => 'https://i3.ytimg.com/vi/' . $video['youtube_id'] . '/0.jpg',
'contentUrl' => 'https://youtube.googleapis.com/v/' . $video['youtube_id'],
'embedUrl' => 'https://www.youtube.com/embed/' . $video['youtube_id'] . '?rel=0',
];
}
}
}
