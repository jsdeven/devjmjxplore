...
class Category extends AEntity
{
    public function index($id = 0, $params = array())
    {
        ...
        if (!empty($id)) {
            ...
            if(empty($data['selected_views'])){
                $data['selected_views'] = null;
            }
            if(empty($data['main_pair'])){
                $data['main_pair'] = null;
            }
            //Get All Sub categories
            $params['status'] = 'A';
            $sub_categories = fn_get_subcategories($id, $params, $lang_code);
            if(!empty($sub_categories)){
                foreach($sub_categories as $sub_category){
                    if(empty($sub_category['main_pair'])){
                        $sub_category['main_pair'] = null;
                    }
