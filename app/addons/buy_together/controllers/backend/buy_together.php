...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update') {
        ...
        return array(CONTROLLER_STATUS_OK, 'jmj_products.add?step=7&product_id=' . $_REQUEST['product_id']);
    }
    ...
    if ($mode == 'delete') {
        if (!empty($_REQUEST['chain_id'])) {
            ...
            return array(CONTROLLER_STATUS_REDIRECT, 'jmj_products.add?step=7&product_id=' . $product_id);
        }
    }
