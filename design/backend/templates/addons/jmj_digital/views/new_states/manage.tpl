...
{$has_permission = fn_check_permissions("new_states", "update_status", "admin", "POST", ["table" => "states"]) && fn_check_permissions("new_states", "m_delete", "admin", "POST", ["table" => "states"])}
    ...
    {capture name="states_table"}
        <div class="table-responsive-wrapper longtap-selection">
            ...
            >
                ...
                <td width="30%" data-th="{__("state")}">
                    <input type="text" name="states[{$state.state_id}][state]" size="55" value="{$state.state}" class="input-hidden"/>
                </td>

            
                <td width="5%" data-th="{__("indiazone")}">
                    <select id="indiazone" name="states[{$state.state_id}][indiazone]" class="input-hidden" required> 
                        <option value="">&nbsp;</option>
                        {foreach from=$india_zones item=india_zone}
                            <option value="{$india_zone.value}" {if $india_zone.value==$state.indiazone}selected="selected"{/if}>{$india_zone.name}</option>
                        {/foreach}    
                    </select>
                </td>
                <td width="5%" class="nowrap" data-th="{__("tools")}">
                    {capture name="tools_list"}
                        <li>{btn type="list" class="cm-confirm" text=__("delete") href="new_states.delete?state_id=`$state.state_id`&country_code=`$search.country`" method="POST"}</li>
                    {/capture}
...
{capture name="tools"}
    ...
    <div class="buttons-container">
        {include file="buttons/save_cancel.tpl" create=true but_name="dispatch[new_states.update]" cancel_action="close"}
    </div>
...
{capture name="buttons"}
    ...
    {if $states}
        {include file="buttons/save.tpl" but_name="dispatch[new_states.m_update]" but_role="action" but_target_form="states_form" but_meta="cm-submit"}
    {/if}
{/capture}

{capture name="adv_buttons"}
    {include file="common/popupbox.tpl" id="new_state" action="new_states.add" text=$title content=$smarty.capture.add_new_picker title=__("add_state") act="general" icon="icon-plus"}
{/capture}
...
<div class="sidebar-field">
    ...
    {include file="buttons/search.tpl" but_name="dispatch[new_states.manage]" method="GET"}
</form>
</div>
{/capture}
