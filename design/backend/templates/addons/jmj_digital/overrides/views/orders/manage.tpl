...
<ul class="nav nav-pills" style="margin-bottom: 15px;">
    {if $approve_order_permission || $process_approved_order_permission}
        <li id="tab_recent_O" class="{if !$_REQUEST.status}active{/if} btn-group">
            <a class="btn" href="{"orders.manage"|fn_url}">{__('All Orders')}</a>
        </li>
        {if $approve_order_permission}
            <li id="tab_recent_O" class="{if $_REQUEST.status == 'O'}active{/if} btn-group">
                <a class="btn" href="{"orders.manage&status=O"|fn_url}">{__('New Orders')}</a>
            </li>
            <li id="tab_recent_O" class="{if $_REQUEST.status == 'E'}active{/if} btn-group">
                <a class="btn" href="{"orders.manage&status=E"|fn_url}">{__('Disapproved')}</a>
            </li>
        {/if} 
        <li id="tab_recent_O" class="{if $_REQUEST.status == 'A'}active{/if} btn-group">
            <a class="btn" href="{"orders.manage&status=A"|fn_url}">{__('Approved')}</a>
        </li>   
        {if $process_approved_order_permission}
            <li id="tab_recent_O" class="{if $_REQUEST.status == 'G'}active{/if} btn-group">
                <a class="btn" href="{"orders.manage&status=G"|fn_url}">{__('Process Orders')}</a>
            </li>
        {/if}    
    {/if}   
</ul>  
    ...
    {capture name="orders_table"}
        <div class="table-responsive-wrapper longtap-selection">
            ...
            <tr>
                ...
                <th width="27%"><a class="cm-ajax" href="{"`$c_url`&sort_by=customer&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("customer")}{if $search.sort_by == "customer"}{$c_icon nofilter}{/if}</a></th>
