        ...
        <tr id="feature_{$id}_{$num}">
            <td colspan="6">
                ...
                *}
                ...
                {assign var="companies" value=fn_get_companies_for_brand()}
                {if $feature.feature_style == 'brand'}
                    <div class="control-group">
                        <label for="company" class="control-label">{__("company")}:</label>
                        <div class="controls">
                            <select id="company" name="feature_data[variants][{$num}][company_id]">
                                <option value="0">{__("none")}</option>
                                {foreach from=$companies item=company}
                                    <option value="{$company.company_id}" {if $company.company_id == $var.company_id}selected="selected"{/if}>{$company.company}</option>
                                {/foreach}    
                            </select>
                        </div>
                    </div>
